...
if (!questions.length) {
  questions = [
    ...
    '3. What decision or conversation is most urgent?'
      ...
      let html = '<div class="jd-answer">';
      html += '<div class="jd-section">';
      html += '<div class="jd-section-title">CLARIFYING QUESTIONS</div>';
      html += '<p class="jd-paragraph">Before I give you a full recommendation, I want to sharpen the situation in three areas:</p>';
      html += '<ol class="jd-list">';
      questions.forEach(q => {
        html += `<li>${escapeHtml(q).replace(/^\d+\.\s*/, '')}</li>`;
      });
      html += '</ol>';
      html += '<p class="jd-paragraph">You can answer these now, or choose Quick Answer and I will move directly to a first-pass recommendation.</p>';
      html += '</div>';
      html += '</div>';

     const rendered = createMessage('assistant', html, true);
renderQuickAnswerButton(rendered.bubble);
forceSectionTop(rendered.row);

const input = document.getElementById('userInput');
if (input) {
  input.value = questions.join('\n\n');
...
if (systemStatus && data?.session?.user?.email) {
  ...
  if (hasActivePaid) {
    ...
    if (tier === 'pro') {
      ...
      systemStatus.innerText = 'JD Lite Active';
    } else {
